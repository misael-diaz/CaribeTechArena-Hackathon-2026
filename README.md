...
Was choosing Python for fast development the right call? It did allowed for rapid writing of skills for the chatbot but that was mainly it. Even though the challenge was about developing a LLM backed chatbot I did not see a real reason to use an LLM for performance and reliability reasons. A NLP algorithm can consistently determine the intent (no hallucination problems), does not require tokens, and does not require much computing compared to running even a ligthweight LLM. And this is reinformed by the fact that the judges did not even bothered asking if anyone used an LLM they just assumed but the code was not part of the evaluation scheme anyways. Had we presented a chatbot that uses NLP that would report the nutrition and what did the children ate they would have been content with just that. This supports my criticism of quickly reaching out for whatever is trendy as a solution, as if a hammer is the only tool that carpenteers have at their disposal.

Going back to was Python the right tool. We ended up with a buggy and slow dashboard that would throw an internal server error randomly (HTTP 500). Was it the right call to agree with Juan, our full stack engineer, to dictate the stack? Yes it was and I am convinced that was the right move. It allowed us to present something even if buggy but functional. Would I be inclined to use Python for future hackathons or for rapid development probably not, but not because of what you may think. The problem with modern web development is the reliance of bloated frameworks that require a lot of effort to deploy. Just look at the project requirements, and if you add the common dependency and versioning issues that Python projects rapidly experience you know that it's probably not the best option. What was the value of following along, confirm that indeed Python has the same problems that have deterred me from using it for web development even if it is great for data processing (by leveraging numpy) and visualization. What is the better alternative, Golang of course.

If you were convinced that Golang was the better alternative why you did not insist. Frankly I wanted to see how would a full stack engineer would handle the problem and see if I was missing something key here. You don't know unless you try and explore this in earnest adn that's what we did. It also helps build rapport for a team that was formed in the eleventh hour. At that moment it was more important to have that than the best stack.

Did I know enough Golang to take the lead? Clearly was not the case on this event. It requires maturity to weight what's important and to me the team was more important, the learning was more important, the exploration was more important, and giving Python a chance was more important than being right. And even if I could have proved that Golang was the right tool I would have missed the experiences that I had. I must say that it will have to wait until I am fully ready.
